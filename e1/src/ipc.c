...
static inline void devq_push(uint32_t base, uint32_t cant)
{
    ...
    if (ids_estado->devq_len >= DEVQ_CAP) {
        // fallback raro: no perder → reinyectar como “nuevos”
...
{
    if (cola)       munmap(cola, sizeof(cola_t));
    if (ids_estado) munmap(ids_estado, sizeof(ids_t));
...
{
    if (!ids_estado) return;
    while (sem_wait(sem_ids) == -1 && errno == EINTR) {}

    if (nprods > MAX_PRODS) nprods = MAX_PRODS;
...
void ipc_mark_dead(pid_t pid)
{
    ...
    for (int i = 0; i < ids_estado->nprods; ++i) {
        if (ids_estado->pid[i] == pid && ids_estado->alive[i]) {
            ids_estado->alive[i] = 0;

            // Si tenía un bloque vigente, devolver lo que falta
...
{
    if (!ids_estado) return 0;
    while (sem_wait(sem_ids) == -1 && errno == EINTR) {}

    // Si hay pendientes y hay menos vivos que nprods, decimos “sí”
    uint32_t pend = ids_estado->restantes;
    for (int i = 0, k = ids_estado->devq_len, p = ids_estado->devq_head; i < k; ++i) {
        pend += ids_estado->devq[p].cant;
        p = (p + 1) % DEVQ_CAP;
    }
    int n = ids_estado->nprods, vivos = 0;
    for (int i = 0; i < n; ++i) vivos += ids_estado->alive[i];

    int v = (pend > 0 && vivos < n) ? 1 : 0;
    sem_post(sem_ids);
    return v;
}
...
int ipc_prods_vivos(void)
{
    ...
    while (sem_wait(sem_ids) == -1 && errno == EINTR) {}
    int vivos = 0;
    for (int i = 0; i < ids_estado->nprods; ++i) vivos += ids_estado->alive[i];
    sem_post(sem_ids);
    return vivos;
}
...
int pop_timeout(registro_t *r, int timeout_ms)
{
    ...
    int rc;
    do { rc = sem_timedwait(sem_full, &ts); } while (rc == -1 && errno == EINTR);

    if (rc == -1) { if (errno == ETIMEDOUT) return 1; return -1; }
...
{
    // Esperar espacio con timeout sobre EMPTY
    struct timespec ts;
    if (clock_gettime(CLOCK_REALTIME, &ts) < 0) return -1;
    ...
    int rc;
    do { rc = sem_timedwait(sem_empty, &ts); } while (rc == -1 && errno == EINTR);
    if (rc == -1) { if (errno == ETIMEDOUT) return 1; return -1; }
...
// ===== IDs: RR estricto con prioridad a devoluciones =====
...
int pedir_bloque_ids_rr(int idx, uint32_t *base, uint32_t *cant)
{
    for (;;) {
        ...
        // Primero reasignar devoluciones si existen
        if (ids_estado->devq_len > 0) {
            ...
            *base = b; *cant = give;
            ...
            // Si sobran en el rango devuelto, reencolar el resto
            if (c > give) devq_push(b + give, c - give);

            // Registrar bloque vigente del idx
        ...
        // Si no hay devoluciones, asignar “nuevos”
            ...
            ids_estado->alive[idx])
        {
            ...
            *cant = give;
            ...
            // Avanzar turno (saltando muertos si quedaran IDs)
            if (ids_estado->nprods > 0) {
                do {
                    ids_estado->turno = (ids_estado->turno + 1) % ids_estado->nprods;
        ...
        // no es mi turno: soltar lock y esperar breve
        sem_post(sem_ids);
