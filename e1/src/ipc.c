...
static int crear_shm(const char* nombre, size_t tam, int crear) { // se traduce como mapear y crear si es necesario
...
int ipc_abrir_todos(int crear, uint32_t total_ids) { // crear =1 para crear, 0 para abrir
    // SHM cola
...
void push(const registro_t* r) { // bloquea hasta que haya espacio
...
int pop(registro_t* r) {
    ...
    if (sem_trywait(sem_full) != 0) 
        return -1; // no hay elementos ocupados
    // hay al menos un elemento, puedo sacar
    sem_wait(sem_mutex); // P
    *r = cola->buffer[cola->primero];
    cola->primero = (cola->primero + 1) % COLA_CAP; // avanza el índice circular
    cola->cant_elem_ocupados--; // un elemento menos
