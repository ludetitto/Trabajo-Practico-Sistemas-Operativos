// Generador: pide bloques de hasta 10 IDs en RR (prioriza devoluciones),
// produce 1 registro por ID y tras cada push notifica ipc_avance_bloque(idx).
    ...
    while (!g_stop)
    {
        ...
        for (uint32_t i = 0; i < cont && !g_stop; ++i)
        {
            ...
            while (!g_stop) {
                ...
                if (prc == 0) { pushed = 1; break; }
                if (prc == 1) { continue; } // timeout: reintentar
            ...
            // Notificar avance en el bloque vigente (clave para reasignación sin huecos)
