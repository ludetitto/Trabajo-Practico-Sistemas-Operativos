...
{
    const char *p = buf; size_t faltan = n;
    while (faltan > 0) {
...
int main(int argc, char **argv) 
{
    ...
    if (connect(sockfd, (struct sockaddr *)&sa, sizeof(sa)) < 0) { perror("connect"); return 1; }

    /* banner inicial */
    if (leer_linea(sockfd, buf, sizeof(buf)) > 0) printf("%s\n", buf);
    ...
    for (;;) {
        ...
        if (len != 0) {
            if (linea[len - 1] != '\n') linea[len++] = '\n';
            if (escribir_todo(sockfd, linea, len) < 0) { perror("write"); break; }

            /* lee 1ra línea de respuesta */
            if (leer_linea(sockfd, buf, sizeof(buf)) <= 0) break;
            printf("%s\n", buf);

            /* si es respuesta multirenglón (ROW ...), seguir hasta END */
