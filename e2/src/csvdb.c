...
int buscar_todos(registro_t **vec_out, size_t *n_out) {
    FILE *f = fopen(csv_path, "r"); // Usar csv_path como ruta global
    if (!f) return -1;

    size_t cap = 32, n = 0;
    registro_t *vec = malloc(cap * sizeof(registro_t));
    if (!vec) { fclose(f); return -1; }

    char linea[512];
    while (fgets(linea, sizeof(linea), f)) {
        registro_t r = {0};
        // Saltear encabezado y líneas borradas
        if (strstr(linea, "id,") == linea) continue;
        if (parsear_linea(linea, &r) == 0 && !r.borrado) {
            if (n == cap) {
                cap *= 2;
                registro_t *tmp = realloc(vec, cap * sizeof(registro_t));
                if (!tmp) { free(vec); fclose(f); return -1; }
                vec = tmp;
            }
            vec[n++] = r;
        }
