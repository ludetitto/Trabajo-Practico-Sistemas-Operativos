...
void procesar_linea_protocolo(int cfd, const char *linea)
{
    ...
    if (!strcmp(cmd, "ORDER")) {
        char by[16], campo[32], modo[8] = "ASC";
        if (sscanf(pbuf, "BY %15s %7s", campo, modo) < 1) {
            dprintf(cfd, "ERR ARG\n"); return;
        }
        if (strcasecmp(campo, "precio") != 0) {
            dprintf(cfd, "ERR ARG\n"); return;
        }
        // Buscar todos los registros
        if (buscar_todos(&vec, &n) != 0 || n == 0) {
            dprintf(cfd, "END\n"); return;
        }
        // Ordenar por precio
        int desc = !strcasecmp(modo, "DESC");
        for (size_t i = 0; i < n; ++i) {
            for (size_t j = i + 1; j < n; ++j) {
                if ((desc && vec[i].precio < vec[j].precio) ||
                    (!desc && vec[i].precio > vec[j].precio)) {
                    registro_t tmp = vec[i];
                    vec[i] = vec[j];
                    vec[j] = tmp;
                }
            }
    ...
    /* ----------- WHERE precio <op> <valor> ----------- */
    if (!strcmp(cmd, "WHERE")) {
        char campo[32], op[4];
        float valor;
        if (sscanf(pbuf, "%31s %3s %f", campo, op, &valor) < 3) {
            dprintf(cfd, "ERR ARG\n"); return;
        }
        if (strcasecmp(campo, "precio") != 0) {
            dprintf(cfd, "ERR ARG\n"); return;
        ...
        for (size_t k = 0; k < n; ++k) {
            int match = 0;
            if (!strcmp(op, "="))      match = (vec[k].precio == valor);
            else if (!strcmp(op, "<")) match = (vec[k].precio < valor);
            else if (!strcmp(op, ">")) match = (vec[k].precio > valor);
            else if (!strcmp(op, "<=")) match = (vec[k].precio <= valor);
            else if (!strcmp(op, ">=")) match = (vec[k].precio >= valor);
            if (match)
