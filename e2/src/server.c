...
static void *iniciar_thread_cliente(void *arg)
{
  ...
  dprintf(cfd, "Conectado. Comandos: PING | GET <id> | FIND <nombre> | FIND ALL <nombre> | MODIFY <id> <NOMBRE|PRECIO|STOCK> <valor> | ADD ... | UPDATE ... | DELETE <id> | BEGIN | COMMIT | ROLLBACK | QUIT\n");
  ...
  while (fgets(linea, sizeof(linea), arch))
  {
    ...
    if (!strncasecmp(linea, "COMMIT", 6)) {
      if (!tx_active) {
        ...
        /* confirmar: descartar snapshot in-memory (commit) y liberar lock */
        (void)guardar_snapshot();
        (void)finalizar_tx();
        dprintf(cfd, "OK\n");
      }
      continue;
    ...
    /* Si hay transacción activa, nadie puede consultar/modificar salvo el dueño */
    pthread_mutex_lock(&tx_mtx);
