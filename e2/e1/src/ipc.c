...
int ipc_abrir_todos(int crear, uint32_t total_ids)
{
    ...
    if (crear)
    {
        ...
        // 🔹 Inicializar todos los generadores como vivos y con contador 0
        for (int i = 0; i < MAX_PRODS; ++i)
        {
            ids_estado->alive[i] = 1;
            ids_estado->producidos[i] = 0;
        }
    }
...
// ============================================================================
// Marca un generador como muerto y devuelve al pool los IDs no generados
// ============================================================================

void ipc_mark_dead(pid_t pid)
{
    if (!ids_estado || !sem_ids)
        return;
    ...
    for (int i = 0; i < ids_estado->nprods; ++i)
    {
        if (ids_estado->pid[i] == pid && ids_estado->alive[i])
        {
            ...
            uint32_t producidos = ids_estado->producidos[i];
            uint32_t faltan = 0;

            // Cada bloque son 10 IDs (según enunciado)
            if (producidos % 10 != 0)
                faltan = 10 - (producidos % 10);

            // Devolver al pool global los IDs pendientes
            ids_estado->restantes += faltan;

            fprintf(stdout,
                    "[MAIN] Generador %d (pid=%d) murió. "
                    "Producidos=%u → faltaban=%u → devueltos. "
                    "Total restantes=%u\n",
                    i, (int)pid, producidos, faltan, ids_estado->restantes);
            fflush(stdout);
